  ...
  while (rclcpp::ok())
  {
    ...
    if (head_type[0] == TYPE_IMU)
    {
      ...
      Eigen::Quaterniond q_xiao_rr =
        ...
        // 1. 先保留原始坐标变换
        Eigen::Quaterniond q_transformed = q_r * q_ahrs * q_rr;
        // 2. 在变换后的姿态上叠加航向偏移（绕最终Z轴）
        Eigen::Quaterniond q_yaw_offset(Eigen::AngleAxisd(0,Eigen::Vector3d::UnitZ()));//无添加偏移
        Eigen::Quaterniond q_out = q_transformed * q_yaw_offset; // 先变换，再偏移
     
      if (device_type_ == 0)         //未经变换的原始数据
      {
        //RCLCPP_INFO(this->get_logger(), "Using device_type_ 0 (原始数据，无偏移)");
      ...
      {
        //RCLCPP_INFO(this->get_logger(), "Using device_type_ 1 (应用偏移)");
        //Eigen::Quaterniond q_out =  q_r * q_ahrs * q_rr;//老的未变换的输出
    ...
    else if (head_type[0] == TYPE_AHRS)
    {
      ...
      // 原始航向角（带偏移）
     // double raw_yaw = ahrs_frame_.frame.data.data_pack.Heading;
  
      // 转换到±π弧度（±180度）范围
    //  double converted_yaw = raw_yaw;
     // if (converted_yaw > M_PI) {
      //  converted_yaw -= 2 * M_PI;  // 大于180度时，减去360度（2π弧度）
     // } else if (converted_yaw < -M_PI) {
     //   converted_yaw += 2 * M_PI;  // 特殊情况：若原始角度小于-180度，加360度（实际较少见）
     // }
